...
class TensorboardVisualizerCallback(Callback):
    ...
    def _apply_mask_overlay(self, image, mask, color=255):
        # mask = np.dstack((mask, mask, mask)) * np.array(color)  ## This is for RGB
        mask = np.expand_dims(mask, axis=2) * np.array(color)
    ...
    def _get_mask_representation(self, image, mask):
        ...
        # Need to conert to grayscale
        print('_get_mask_representation')
        print(image.shape)
        print(mask.shape)
        H, W, C = image.shape
        # results = np.zeros((H, 3 * W, 3), np.uint8)
        # p = np.zeros((H * W, 3), np.uint8)
        results = np.zeros((H, 3 * W, 1), np.uint8)
